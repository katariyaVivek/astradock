...
| `cpp/estimation` | Navigation layer: fixed-size `math::Matrix<R,C>` with Cholesky SPD solves, generic Joseph-form updates with NIS/NEES diagnostics, analytical gravity Jacobian with finite-difference audits, first-order discrete transition $\Phi \approx I + F\Delta t$, coupled process noise $Q$, GNSS measurement model ($H_{\text{GNSS}}$), star tracker measurement model ($H_{\text{ST}}$ with double-cover alignment), relative range measurement model ($H_{\text{range}}$ with singularity guard), 6-state `TranslationalEkf`, 6-state `AttitudeEkf`, IMU-aided specific-force dead reckoning, and the unified 15-state `IntegratedNavigationEkf` ($\delta\mathbf{x} = [\delta\mathbf{r}^T, \delta\mathbf{v}^T, \delta\boldsymbol{\theta}^T, \delta\mathbf{b}_a^T, \delta\mathbf{b}_g^T]^T \in \mathbb{R}^{15}$) with full cross-covariance coupling (attitude/accel-bias to velocity) and first-order covariance reset $J_{\text{reset}}$ | Guidance, control, desaturation controllers |
| `cpp/actuators` | Reaction-wheel dynamics (momentum exchange, torque/speed saturation, motor lag), thruster force/torque geometry with ECI rotation, propellant mass flow with dry-mass floor, wheel/thruster assembly aggregation | Guidance, control, desaturation controllers |
| `cpp/relative` | Target/chaser LVLH relative state, exact ECI round trip, CW closed-form prediction and forced RK4 propagation (circular reference, documented breakdown) | Eccentric/drag-differential extensions, guidance use (M18+) |
| `cpp/control` | Quaternion-error PD + rate damping, CW-feedforward relative PD, closed-form double-integrator LQR, desired/achieved saturation accounting (open-loop laws; no estimator wiring) | Integral action (later) |
| `cpp/gnc` | Estimates-only attitude loop seam (estimate-based tick, reference timeline, latching metrics), wheel-assembly actuation, MEKF-in-loop | Docking (M19+) |
| `cpp/rendezvous` | Waypoint sequencing, closing-speed profiles, keep-out/corridor/speed predicates, thrust-aware EKF predict, epoch-aligned ECI scenarios | Retreat trajectories (M21+) |
| `cpp/docking` | Body-frame port geometry, axial penalty contact, all-criteria acceptance + latching, deterministic abort codes | Retreat trajectories, tumbling target (M21+) |
| `cpp/fdir` | Deterministic fault injection, M-of-N NIS/residual monitors, worst-margin isolation, exclusion/coast/safe-mode policy | Multi-fault combinatorics, ML detection (M23) |
| `cpp/mission` | Config-as-data scenarios, FNV-1a run identity, phase timelines, seeded Monte Carlo + percentiles, regression scorecards | File-format configs, parallel runner (later) |
| `cpp/vision` | Pinhole camera + fiducials, Huber-LM PnP with inlier gating (pose estimates only, not fused) | Learned models, nav fusion (M23+) |
| `ml` (Python) | Seed-family-split NIS-window datasets, M20/logreg baselines, kNN challenger, advisory-only integration (no C++ loop path) | Learned pose, closed-loop ML (never without safety case) |
| `cpp/environment` | High-fidelity upgrades: ECI/ECEF rotation, JD time, Bowring geodetic, J2–J4 total gradient, Moon/Sun ephemeris (educational atmosphere retained) | Operational atmosphere, EOP/leap-second time (documented non-goals) |
| `capstone` (tools) | 9-phase end-to-end mission on verified parts, scored per phase, 100-run Monte Carlo (no new filters/controllers — composition only) | New development (project closed) |
